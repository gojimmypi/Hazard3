...
A `1` bit indicates that interrupt is currently asserted. IRQs are assumed to be level-sensitive, and the relevant `meipa` bit is cleared by servicing the requester so that it deasserts its interrupt request.
...
Index `0x402` contains the Hazard3 version:
...
| 23:16  | `minor` |
| 15:8   | `patch` |
| 7:0    | `eco`   | Connected to external signals
|===

This should match the version of the Github release of the processor's source code. For example, version 1.0.2 would have a `major` of 1, `minor` of 0 and `patch` of 2. `eco` is a special field which is brought out to an input at the Hazard3 top level; you can connect this to metal tie cells in your integration so that you can increment the ECO (engineering change order) number with metal revisions that affect the processor, or you can simply tie it to 0.
