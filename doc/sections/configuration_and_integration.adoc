...
The core waits for all in-progress bus accesses (load/store and instruction fetch) to complete before asserting this signal.
...
The core waits for any in-progress load/store bus accesses to complete before asserting this signal.
...
Requires: <<param-EXTENSION_ZBB>> = `1`. (Since Zbb and Zbkb have a large overlap, this flag enables only those instructions which are in Zbkb but aren't in Zbb. Therefore both flags must be set for full Zbkb support.)
...
Requires: <<param-EXTENSION_C>> = `1`. (Some Zcb instructions also require Zbb or M, as they are 16-bit aliases of 32-bit instructions present in those extensions.)
