...
Hazard3 supports ASIC synthesis using common commercial tool flows. There are no particular requirements for configuration parameters, but your choice of configuration has an impact on area and frequency. Please raise an issue https://https://github.com/Wren6991/Hazard3/issues[here] if you find a compatibility issue with your tools.
...
4+| These signals are used in the implementation of internal sleep states as configured by the <<reg-h3.msleep>> csr. They are used only when the Xh3power extension is enabled.
...
4+| [[memory-ordering-signals]] **Memory Ordering Signals**
...
Setting this flag to zero makes the core incapable of handling exceptions and interrupts. Instructions which would raise exceptions, such as illegal opcodes or unaligned load/store addresses, instead execute as NOPs.
...
Ignored if <<param-EXTENSION_XH3IRQ>> = `0`. In this case, when the internal
interrupt controller is not present, all external IRQ inputs are OR'd together
before being registered in a single flip-flop.
...
Use a single-cycle multiply circuit for MUL instructions, retiring to stage 3.
...
NOTE: The vectoring mode, in bits `1:0`, can be made fixed by clearing the LSB of `MTVEC_WMASK`.
...
Hazard3 implements one or two AHB5 bus manager ports. <<port-descriptions>> describes their signal-level implementation. This section describes their behaviour at a higher level, and the requirements Hazard3 imposes on your bus subsystem.
...
All Hazard3 bus accesses are naturally aligned. That is, address modulo access size is always zero. Additionally, instruction fetch is always word-aligned and word-sized.
...
Hazard3 expects that all memory used for instruction fetch is read-idempotent. This limitation comes about because PMP execute permissions are checked at the point an instruction is _executed_, not the point it is fetched. This is a deliberate design decision that reduces the logic depth of fetch address generation logic. As a consequence it is possible to trigger reads from arbitrary addresses by jumping to them.

This means you must not permit instruction fetch from IO regions if <<param-U_MODE>> is configured, because this would allow U-mode software to cause IO read side effects on privileged registers by (fatally) jumping to an IO address. This can be enforced in one of two ways:
