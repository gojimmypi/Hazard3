...
All interrupts are level-sensitive and synchronous to `clk`. Asynchronous interrupts must be synchronised externally before entering the processor. If an interrupt is used for waking from sleep states, make sure to use the ungated version of `clk` (`clk_always_on`) for synchronisation.
