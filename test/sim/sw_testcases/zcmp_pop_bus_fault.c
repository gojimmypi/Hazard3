...
// cm.pop results in the expected mcause, mepc and GPR values.
...
void test_popret(int poison_idx) {
	...
	// Return address is two instructions after the pop
	...
	asm volatile (
		...
		"sw sp, (a0)\n"
		"la sp, stackframe\n"
		// Clear regs loaded by pop, so that if they have the correct value
