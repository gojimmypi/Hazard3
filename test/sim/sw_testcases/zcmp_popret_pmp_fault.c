...
// Test that a PMP read permission failure occurring at each address loaded by
// a maximal Zcmp cm.popret results in the expected mcause, mepc and GPR
// values.
...
// Conditionally use PMP region 0 to remove read permissions from a 1-word
// region of memory:
void pmp_poison_addr(uintptr_t addr, bool poison) {
	write_csr(pmpcfg0, 0);
	write_csr(pmpaddr0, addr >> 2);
...
void test_popret(int poison_idx) {
	...
	if (poison_idx >= 0) {
		pmp_poison_addr((uintptr_t)&stackframe[3 + poison_idx], true);
	}
	...
	asm volatile (
		...
		"sw sp, (a0)\n"
		"la sp, stackframe\n"
		// Clear regs loaded by popret, so that if they have the correct value
	...
	"popret_instr:\n"
		...
		:
		:
		// Trashed by cm.popret and our setup
...
void handle_exception_impl(uint32_t a0, uint32_t sp) {
	...
	tb_assert(a0 == -1, "popret should not clear a0: %08x\n", a0);
